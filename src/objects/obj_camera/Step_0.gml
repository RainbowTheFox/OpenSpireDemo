...
if (instance_exists(obj_parent_player) && obj_parent_player.state != 11 && obj_parent_player.state != 19)
{
	var target = 
	{
		x: global.targetCamX,
	...
	if (!global.freezeframe)
	{
		...
		if (_player.state == 32 || _player.state == 33 || _player.state == 61 || _player.state == 63)
		{
			_targetcharge = _player.xscale * ((_player.movespeed / 4) * 50);
			_tspeed = 0.3;
			
			if (sign(chargeCameraX) != _player.xscale || (_player.state == 61 || _player.state == 63))
				_tspeed = 3;
		}
		else if (abs(_player.hsp) >= 16 && _player.state != 6 && _player.state != 27)
		{
