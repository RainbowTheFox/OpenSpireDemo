...
if (doCollision && state != 5 && state != 7 && state != 8)
	scr_collision();

canBreakBlocks = false;

if (global.freezeframe && state != 9)
{
	frozenState = state;
...
if (tauntBuffer)
{
	if (obj_parent_player.state != 18 && obj_parent_player.state != 51)
	{
		tauntBuffer = false;
		baddieStunTimer = 0;
		baddieScareBuffer = 0;
...
{
	create_afterimage(7, image_xscale);
