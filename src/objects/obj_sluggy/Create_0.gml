...
jumpEvent = function(argument0 = true)
{
	...
	sprite_index = (state == 100) ? spr_sluggy_undergroundjumpstart : spr_sluggy_jumpstart;
	image_index = 0;
	burrowTimer = 0;
	state = 2;
};
...
{
	if (enemyAttackTimer <= 0 && scr_enemy_playerisnear(200, 200, undefined, 16) && grounded && (state == 100 || state == 0))
		jumpEvent();
};
