...
if (other.dashpadBuffer <= 0)
{
	with (other)
	{
		if (state != 28 && state != 61 && state != 29 && state != 42 && state != 41 && sprite_get_bbox_bottom(mask_index) <= other.bbox_bottom)
		{
			movespeed += 0.25;
			movespeed = clamp(movespeed, 12, 24);
			...
			if (global.playerCharacter == 0)
			{
				...
				if (state != 6 && !scr_solid(other.x, y))
					x = other.x;
		...
		else if (state == 61 && sprite_get_bbox_bottom(mask_index) <= other.bbox_bottom)
		{
			xscale = sign(other.image_xscale);
			movespeed += 4;
