switch (state)
{
	case 0:
		...
		else
		{
			...
			if (character == 4 && !global.panic && sprite_index != spr_builderBear_karen_idleAnim && choose(1, 0))
			{
				image_index = 0;
		...
		if (character == 3 || character == 4 || (character == 2 && global.panic))
			movespeed = 0;
		...
		if (get_nearestPlayer(x, y).state == 18 && state != 1 && bbox_in_camera(self, view_camera[0], 50))
		{
			state = 1;
			scr_taunt_storeVariables();
			
			if (character == 4 && obj_parent_player.x != x)
