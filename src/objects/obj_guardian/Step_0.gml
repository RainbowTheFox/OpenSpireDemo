...
switch (state)
{
	case 0:
		...
		if (((target_player.cutscene || target_player.state == 55) && !awoken) || offended)
			exit;
...
with (target_player)
{
	if ((sprite_index == spr_supertaunt1 || sprite_index == spr_supertaunt2 || sprite_index == spr_supertaunt3 || sprite_index == spr_supertaunt4) && state == 18)
		player_supertaunt = true;
}

if (state != 4 && state != 0 && player_supertaunt && bbox_in_camera(id, view_camera[0]))
{
	state = 4;
	offended = true;
	sprite_index = spr_aim;
...
if (target_player.state == 59 || target_player.state == 93)
{
	state = 4;
	movespeed = 0;
