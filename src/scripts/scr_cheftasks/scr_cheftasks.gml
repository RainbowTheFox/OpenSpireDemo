...
function ExhibitionNightSecretTask(argument0, argument1, argument2) : OutfitTask(argument0, argument1, 1, -4) constructor
{
...
{
	ini_open(global.SaveFileName);
	var prevUnlocked = ini_read_real("ChefTasks", argument0, false);
	ini_write_real("ChefTasks", argument0, 1);
	ini_close();
	obj_hudManager.saveAlpha = 10;
	
	if (!prevUnlocked)
		ds_queue_enqueue(obj_achievementTracker.chefUnlockQueue, [argument1, argument0, 0, argument2]);
...
function scr_award_palette(argument0, argument1)
{
	...
	var prevUnlocked = ini_read_real("Palettes", argument0, false);
...
function scr_check_end_level_chef_tasks()
{
	with (obj_achievementTracker)
	{
		for (var i = 0; i < array_length(activeChefTasks); i++)
		{
			...
			if (current_task.conditionType == 1 && current_task.taskCondition())
			{
		...
		for (var p = 0; p < array_length(activeExhibitionNightSecretTasks); p++)
		{
			...
			with (current_task)
			{
				if (conditionType == 1 && taskCondition())
				{
...
function scr_task_notify(argument0, argument1 = [])
{
	...
	with (obj_achievementTracker)
	{
		...
		if (ind >= 0 && task.conditionType == 2 && task.taskCondition(argument1))
		{
			if (palette)
...
{
	var pals = [new ExhibitionNightSecretTask("palette_PZ_exhibitionred", 0, 3), new ExhibitionNightSecretTask("palette_PZ_exhibitionblack", 0, 6), new ExhibitionNightSecretTask("palette_PZ_exhibitionpurple", 0, 9), new ExhibitionNightSecretTask("palette_PZ_noise", 0, 12), new OutfitTask("palette_PZ_exhibitionbrain", 0, 2, function()
	{
		ini_open(global.SaveFileName);
		var judge = ini_read_string("Game", "Judgment", "none");
		ini_close();
		return judge != "none";
	}), new OutfitTask("palette_PZ_exhibitionbraingold", 0, 2, function()
	{
		ini_open(global.SaveFileName);
		var judge = ini_read_string("Game", "Judgment", "none");
		ini_close();
		return judge == "perfect" || judge == "holyshit";
	}), new OutfitTask("palette_PZ_exhibitionpaper", 0, 0, function()
	{
		if (instance_exists(obj_exhibitiontrashcan))
			...
			return false;
	}), new OutfitTask("palette_PZ_exhibitionentryway", 0, 0, function()
	{
		...
		return false;
	}), new OutfitTask("palette_PZ_exhibitionsteamy", 0, 0, function()
	{
	...
	}), new OutfitTask("palette_PZ_exhibitionmineshaft", 0, 0, function()
	{
		if (obj_parent_player.grounded)
			obj_achievementTracker.bouncedOnFred = 0;
		
		return obj_achievementTracker.bouncedOnFred >= 5;
	}), new OutfitTask("palette_PZ_exhibitionmolasses", 0, 0, function()
	{
	...
	{
		ini_open("optionData.ini");
		var _palettes = array_filter(pals, function(argument0, argument1)
		{
...
function scr_get_chef_tasks(argument0, argument1 = true)
{
	...
	{
		case "entryway":
			tasks[0] = new ChefTask("task_cc_all_breakables", 0, spr_bakertasks_entry, 0, function()
			{
			...
			tasks[1] = new ChefTask("task_cc_speedrun", 0, spr_bakertasks_entry, 1, function()
			{
			...
			tasks[2] = new ChefTask("task_cc_no_slip", 1, spr_bakertasks_entry, 2, function()
			{
		...
		case "steamy":
			tasks[0] = new ChefTask("task_ct_clock_room", 0, spr_bakertasks_steamy, 0, function()
			{
				return room == steamy_sideroom;
			});
			tasks[1] = new ChefTask("task_ct_all_witches", 0, spr_bakertasks_steamy, 1, function()
			{
			...
			tasks[2] = new ChefTask("task_ct_few_cotton_blocks", 0, spr_bakertasks_steamy, 2, function()
			{
		...
		case "mineshaft":
			tasks[0] = new ChefTask("task_sm_all_miners", 0, spr_bakertasks_mines, 0, function()
			{
			...
			tasks[1] = new ChefTask("task_sm_minecart_nohit", 1, spr_bakertasks_mines, 1, function()
			{
				return !obj_achievementTracker.hitInMinecart;
			});
			tasks[2] = new ChefTask("task_sm_slug", 2, spr_bakertasks_mines, 2, function(argument0)
			{
		...
		case "molasses":
			tasks[0] = new ChefTask("task_ms_all_geyser", 0, spr_bakertasks_molasses, 0, function()
			{
			...
			tasks[1] = new ChefTask("task_ms_guardian_parry", 0, spr_bakertasks_molasses, 1, function()
			{
				return obj_achievementTracker.guardianSupertaunted >= 3;
			});
			tasks[2] = new ChefTask("task_ms_catch", 0, spr_bakertasks_molasses, 2, function()
			{
				if (obj_parent_player.grounded)
		...
		case "demoEN":
			tasks[0] = new ChefTask("task_en_all_s", 1, spr_bakertasks_en_ranks, 0, function()
			{
			...
			tasks[1] = new ChefTask("task_en_all_p", 1, spr_bakertasks_en_ranks, 1, function()
			{
				var lvlArr = ["entryway", "steamy", "mineshaft", "molasses"];
				var res = true;
				ini_open(global.SaveFileName);
