	...
	switch (object_index)
	{
		...
		case obj_sluggy:
			...
			if (sprite_index == spr_sluggy_underground)
			{
				...
				if (place_meeting_collision(x + image_xscale, y, 1))
				...
				if (distance_to_object(obj_parent_player) < 100 && obj_parent_player.state == 38)
				{
					hsp = 0;
					vsp = -8;
					grounded = false;
				...
				if ((enemyAttackTimer <= 0 || player_present || !place_meeting(x + hsp, y + 1, obj_dirtpatch)) && obj_parent_player.state != 38)
				{
