...
function scr_playersounds()
{
	...
	if (event_instance_isplaying(sndSuplex) && saved_state != 10)
		fmod_studio_event_instance_stop(sndSuplex, true);
	
	if (saved_state == 7 && (sprite_index != spr_wallJumpFastFallIntro && sprite_index != spr_wallJumpFastFall))
	{
	...
	if (saved_state == 61 && grounded && vsp > 0 && sprite_index != spr_player_PZ_minecart_spinOut)
	{
	...
	if ((saved_state == 16 && grounded && vsp > 0) || saved_state == 39)
	{
	...
	if (saved_state == 58 && place_meeting(x, y, obj_cottonsolid))
	{
	...
	if (grounded && saved_state == 41 && movespeed >= 12 && sprite_index != spr_player_PZ_dogMount_skid)
	{
	...
	if (saved_state == 37 || saved_state == 21 || saved_state == 15)
	{
	...
	if ((saved_state != 27 || sprite_index == spr_superjumpCancelIntro) && event_instance_isplaying(sndSuperjumpRelease))
		fmod_studio_event_instance_stop(sndSuperjumpRelease, true);
	
	if (saved_state == 32 || saved_state == 3 || saved_state == 33 || saved_state == 6)
	{
		...
		if ((saved_state == 32 && sprite_index == spr_mach1) || (saved_state == 3 && sprite_index == spr_mach1))
			machsnd = 1;
		else if ((saved_state == 32 && sprite_index == spr_mach2) || (saved_state == 3 && movespeed < 12) || (saved_state == 6 && verticalMovespeed < 12))
			machsnd = 2;
		else if ((saved_state == 33 && sprite_index != spr_crazyrun) || saved_state == 3 || (saved_state == 6 && verticalMovespeed >= 12))
			machsnd = 3;
